...
let me = {
    firstName: 'Triston',
    superHeroName: 'Helios Magas Serapis',
    homeTown: 'Denison',
    superPowers: ['Telepathy', 'Immortality', 'Rain of Arrows'],
...
function setColor(arr){
    let trimmedArr = arr.slice(0, 3)

    for (let i = 0; i < trimmedArr.length; i++){
        if (trimmedArr[i] === 'blue'){
            trimmedArr[i] = '#4D4DFF'
        }
    }

    background(trimmedArr[0], trimmedArr[1], trimmedArr[2])
}
